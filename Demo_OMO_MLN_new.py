...
def Demo():
    ...
    while len(Domains) < N:
        ...
        CurlBounds += [np.sqrt(8)*svds(J,k=1,which='LM',return_singular_vectors=False).item()]
    ...
    np.savez_compressed('NoRegret_MLN_new.npz',opt_d_avg=opt_distances_avg,equi_d_avg=equi_distances_avg,
                        rs_avg=regret_standards_avg,rn_avg=regret_news_avg,stokes_exact=stokes_exact_avg,stokes=stokes_avg)
    ...
    plt.fill_between(ts, regret_news_avg-stokes_exact, regret_news_avg+stokes_exact,
                     facecolor='c', alpha=0.2, zorder=5, label='Stokes Bound (exact)')
    ...
    plt.savefig('NoRegret_MLN_new.pdf',format='pdf',additional_artists=[lgd1,lgd2],bbox_inches='tight')
    ...
    plt.savefig('NoRegret_MLN_new2.pdf',format='pdf',additional_artists=[lgd],bbox_inches='tight')
    ...
    plt.savefig('NoRegret_MLN_new3.pdf',format='pdf',additional_artists=[lgd],bbox_inches='tight')
    ...
    sat_exact = np.logical_or(regret_standards_avg >= regret_news_avg-stokes_exact, regret_standards_avg <= regret_news_avg+stokes_exact)
