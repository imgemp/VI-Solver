...
class CashKarp(Solver):
    ...
    def update(self, record):
        ...
        # Perform update
        _NewData = self.Proj.P(
            Data, Step, np.tensordot(
                self.BT[
                    0, 0:1], Fs[
                    0:1], axes=(
                    0, 0)))
        ...
        if Delta == 0.:
            ...
            Step = max(min(Step * min((self.Delta0 / Delta) ** 0.2,
                                      self.GrowthLimit),
                           self.MaxStep),
                       self.MinStep)
        ...
        TempData['Step'] = Step
        TempData['f Evaluations'] = 6 + self.temp_storage['f Evaluations'][-1]
        TempData['Projections'] = 6 + self.temp_storage['Projections'][-1]
