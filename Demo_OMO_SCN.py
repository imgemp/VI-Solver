...
def Demo():
    ...
    while len(Domains) < N:
        # Create Domain
        Network = CreateRandomNetwork(I=3,Nm=2,Nd=2,Nr=1,seed=None)
        ...
        CurlBounds += [eta*np.sqrt(4.5)*svds(J,k=1,which='LM',return_singular_vectors=False).item()]
    ...
    np.savez_compressed('NoRegret_SCN.npz',d_avg=distances_avg,
                        rs_avg=regret_standards_avg,rn_avg=regret_news_avg,stokes=stokes)
    ...
    plt.title('Demonstration of No-Regret on SCN')
