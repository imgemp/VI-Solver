...
def Demo():
    ...
    scale = np.array([0.4,0.6])
    y = np.sum(scale*X**2,axis=1)
    ...
    # Method = HeunEuler(Domain=LF,Delta0=1e-2,P=P)
    ...
    # # A = np.array([[5,0.],[0.,5]])
    # b = np.zeros(LF.XDim)
    # Start = LF.Ab_to_PDb(A,b,flat=True).astype(np.complex)
    # print(Start)
    ...
    # A,Pinv,D,P,b = LF.ExtractParams(params)
    ...
    xt = LF.x(np.linspace(0.0001,t,20),t0,tf,x0,xf,[A,b])
    plt.plot(x0[0],x0[1],'*')
    ...
    EL = LF.EulerLagrange(np.linspace(0.0001,t,10),t0,x0,y0,tf,xf,[A,b])
    ...
    plt.xlabel(r'$X_0$')
    plt.ylabel(r'$X_1$')
    plt.title('Classical Path over an Affine (Non-Conservative)\n Field Derived from Physical Laws')
    plt.savefig('action.png')


    t = np.linspace(0.0001,t,20)

    L = np.asarray([LF.Lagrangian(_t,t0,x0,y0,tf,xf,[A,b]) for _t in t])
    plt.subplot(2,1,1)
    plt.plot(t,L,marker=(5,3,0),markersize=20)
    plt.xlim([0,t[-1]])
    plt.ylim([-5,25])
    plt.ylabel('Path Lagrangian')
    plt.title('Lagrangian and Prediction Along Path')

    Y = np.asarray([LF.predict([A,b],t0,x0,y0,tf,xf,t=_t) for _t in t])
    plt.subplot(2,1,2)
    plt.plot(t,Y,marker=(5,3,0),markersize=20)
    plt.xlim([0,t[-1]])
    plt.ylim([-12,2])
    plt.xlabel(r'Time [0=start, 1=finish ($\bigstar$)]')
    plt.ylabel('Path Prediction')

    plt.savefig('lag_pred.png')
