...
class Coordination(Domain):
    ...
    def __init__(self):
        ...
        self.b = np.array([-(self.r[1,1]-self.r[0,1]),-(self.c[1,1]-self.c[1,0])])
        self.A_curl = np.array([[2.*self.uprime**2.,0],[0,2.*self.u**2.]])
        self.b_curl = np.array([-2.*self.uprime*(self.c[1,1]-self.c[1,0]),-2.*self.u*(self.r[1,1]-self.r[0,1])])
    ...
    def NE_L2Error(self,Data):
        return np.min(np.array([np.linalg.norm(Data-self.NE[0]),np.linalg.norm(Data-self.NE[1])]))